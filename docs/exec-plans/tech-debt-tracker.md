...
| TD-2 | `docs/architecture.md`(구버전) 모듈 목록/레이아웃 낡음 | 문서 드리프트 | open | 한 사이클 뒤 `ARCHITECTURE.md` 로 통합/대체 |
...
| TD-9 | `docs/medication_api_entity_audit.md` 에 머신 종속 절대경로 링크(`/D:/...`) 다수 | 문서 | open | 상대경로로 교체 또는 제거. 링크체커는 절대경로 skip |
