...
push develop ─> GitHub Actions(deploy-dev) ─> gradle build(-x test)
   ─> Docker build/push (hannah098/attune-be:dev + dev-<commit SHA>)
...
5. 롤백: `deploy-dev` 수동 실행에서 직전의 `dev-<40자리 commit SHA>`를 `image_tag`로 지정해 재배포한다. 이미지는 불변 태그를 사용하므로 `dev` 최신 태그를 직접 롤백 대상으로 쓰지 않는다. 코드 수정 자체를 되돌려야 하면 문제 커밋을 revert 후 재배포한다.
