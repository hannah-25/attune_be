...
public class GeminiTextGenerator implements AiTextGenerator {
        ...
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                if (status == 429) {
                    ...
                    throw new GeminiUnavailableException(GENERATION_FAILED_MESSAGE, sanitizedHttpException(status, e));
                }
                if (status != 502 && status != 503 && status != 504) {
                    // 400/401/403/404/500 등은 재시도 의미 없음 → 즉시 실패(500).
                    throw new GeminiGenerationException(GENERATION_FAILED_MESSAGE, sanitizedHttpException(status, e));
                }
                lastTransient = sanitizedHttpException(status, e); // 게이트웨이/과부하 일시 오류(502·503·504)만 재시도
    ...
    private RuntimeException sanitizedHttpException(int status, Exception e) {
        // RestClientResponseException은 Gemini 응답 body를 보존한다. AI 응답/오류 본문은 로그 경로에 싣지 않는다.
        RuntimeException sanitized = new IllegalStateException("Gemini HTTP " + status);
