package attune.medicationAnalysis.application.engine;

import attune.journal.domain.model.JournalTag;
import attune.journal.domain.model.JournalTagCategory;
import attune.journal.domain.model.JournalTagLog;
import attune.journal.domain.model.JournalTagScope;
import attune.journal.domain.repository.DailyGoalLogRepository;
import attune.journal.domain.repository.DailyStatusLogRepository;
import attune.journal.domain.repository.JournalTagLogRepository;
import attune.journal.domain.repository.JournalTagLogView;
import attune.medication.domain.model.Medication;
import attune.medication.domain.model.MedicationDosage;
import attune.medication.domain.model.UserMedication;
import attune.medication.domain.repository.UserMedicationLogRepository;
import attune.medication.domain.repository.UserMedicationRepository;
import attune.medicationAnalysis.application.model.AnalysisSnapshot;
import org.junit.jupiter.api.Test;

import java.math.BigDecimal;
import java.time.LocalDate;
import java.time.LocalDateTime;
import java.util.List;
import java.util.Map;
import java.util.UUID;

import static org.assertj.core.api.Assertions.assertThat;
import static org.junit.jupiter.api.Assertions.assertEquals;
import static org.mockito.Mockito.mock;
import static org.mockito.Mockito.when;

class MedicationChangeDetectorTest {

    private static final UUID USER_ID = UUID.randomUUID();
    private static final LocalDate PERIOD_START = LocalDate.of(2026, 1, 1);
    private static final LocalDate PERIOD_END = LocalDate.of(2026, 1, 31);

    private final UserMedicationRepository userMedicationRepository = mock(UserMedicationRepository.class);
    private final JournalTagLogRepository journalTagLogRepository = mock(JournalTagLogRepository.class);
    private final DailyStatusLogRepository dailyStatusLogRepository = mock(DailyStatusLogRepository.class);
    private final DailyGoalLogRepository dailyGoalLogRepository = mock(DailyGoalLogRepository.class);
    private final MedicationChangeDetector detector = new MedicationChangeDetector(
            userMedicationRepository,
            mock(UserMedicationLogRepository.class),
            journalTagLogRepository,
            dailyStatusLogRepository,
            dailyGoalLogRepository
    );

    @Test
    void detectClassifiesDifferentPreviousMedicationAsSwitch() {
        UserMedication previous = medication(1L, 10L, 100L, "Previous", "10.00", PERIOD_START.minusDays(1));
        UserMedication current = medication(2L, 20L, 200L, "Current", "20.00", PERIOD_START.plusDays(1));
        when(userMedicationRepository.findAllByUserIdWithDetails(USER_ID))
                .thenReturn(List.of(previous, current));

        AnalysisSnapshot.MedicationChange change =
                detector.detect(USER_ID, PERIOD_START, PERIOD_END).get(0);

        assertEquals("SWITCH", change.changeType());
        assertEquals("Previous", change.previousMedicationName());
    }

    @Test
    void detectClassifiesSameMedicationWithDifferentDosageAsDoseChange() {
        UserMedication previous = medication(1L, 10L, 100L, "Medication", "10.00", PERIOD_START.minusDays(1));
        UserMedication current = medication(2L, 10L, 200L, "Medication", "20.00", PERIOD_START.plusDays(1));
        when(userMedicationRepository.findAllByUserIdWithDetails(USER_ID))
                .thenReturn(List.of(previous, current));

        AnalysisSnapshot.MedicationChange change =
                detector.detect(USER_ID, PERIOD_START, PERIOD_END).get(0);

        assertEquals("DOSE_CHANGE", change.changeType());
    }

    @Test
    void detectClassifiesSameMedicationAndDosageAsContinue() {
        UserMedication previous = medication(1L, 10L, 100L, "Medication", "10.00", PERIOD_START.minusDays(1));
        UserMedication current = medication(2L, 10L, 100L, "Medication", "10.00", PERIOD_START.plusDays(1));
        when(userMedicationRepository.findAllByUserIdWithDetails(USER_ID))
                .thenReturn(List.of(previous, current));

        AnalysisSnapshot.MedicationChange change =
                detector.detect(USER_ID, PERIOD_START, PERIOD_END).get(0);

        assertEquals("CONTINUE", change.changeType());
    }

    @Test
    void detectClassifiesMedicationWithoutPreviousRecordAsAdd() {
        UserMedication current = medication(1L, 10L, 100L, "Medication", "10.00", PERIOD_START.plusDays(1));
        when(userMedicationRepository.findAllByUserIdWithDetails(USER_ID))
                .thenReturn(List.of(current));

        AnalysisSnapshot.MedicationChange change =
                detector.detect(USER_ID, PERIOD_START, PERIOD_END).get(0);

        assertEquals("ADD", change.changeType());
    }

    @Test
    void beforeAfterComparisonGroupsTroubleTagsByNameAndType() {
        UserMedication previous = medication(
                1L, 10L, 100L, "Medication", "10.00", PERIOD_START.minusDays(1));
        LocalDate changeDate = LocalDate.of(2026, 1, 16);
        UserMedication current = medication(
                2L, 10L, 200L, "Medication", "20.00", changeDate);
        LocalDate beforeStart = LocalDate.of(2026, 1, 1);
        LocalDate beforeEnd = LocalDate.of(2026, 1, 15);
        LocalDate afterStart = LocalDate.of(2026, 1, 16);
        LocalDate afterEnd = LocalDate.of(2026, 1, 30);
        when(userMedicationRepository.findAllByUserIdWithDetails(USER_ID))
                .thenReturn(List.of(previous, current));
        when(journalTagLogRepository.findDistinctJournalDatesByUserIdAndJournalDateBetween(
                USER_ID, beforeStart, beforeEnd))
                .thenReturn(List.of(beforeStart, beforeStart.plusDays(1), beforeStart.plusDays(2)));
        when(journalTagLogRepository.findDistinctJournalDatesByUserIdAndJournalDateBetween(
                USER_ID, afterStart, afterEnd))
                .thenReturn(List.of(afterStart, afterStart.plusDays(1), afterStart.plusDays(2)));
        when(journalTagLogRepository.findAllWithTagByUserIdAndJournalDateBetween(
                USER_ID, beforeStart, beforeEnd))
                .thenReturn(List.of(
                        troubleLog(1L, "깜빡함", "INATTENTION", beforeStart),
                        troubleLog(2L, "설명을 놓침", "INATTENTION", beforeStart.plusDays(1))
                ));
        when(journalTagLogRepository.findAllWithTagByUserIdAndJournalDateBetween(
                USER_ID, afterStart, afterEnd))
                .thenReturn(List.of());
        when(dailyStatusLogRepository.findByUserIdAndDateBetween(
                org.mockito.ArgumentMatchers.eq(USER_ID),
                org.mockito.ArgumentMatchers.any(LocalDate.class),
                org.mockito.ArgumentMatchers.any(LocalDate.class)))
                .thenReturn(List.of());
        when(dailyGoalLogRepository.findAllInRangeWithGoal(
                org.mockito.ArgumentMatchers.eq(USER_ID),
                org.mockito.ArgumentMatchers.any(LocalDate.class),
                org.mockito.ArgumentMatchers.any(LocalDate.class)))
                .thenReturn(List.of());

        AnalysisSnapshot.MedicationChange change =
                detector.detect(USER_ID, PERIOD_START, PERIOD_END).stream()
                        .filter(candidate -> candidate.changeDate().equals(changeDate))
                        .findFirst()
                        .orElseThrow();

        AnalysisSnapshot.DayGroupComparison beforeGroup =
                change.beforeAfterComparison().beforeGroupComparisons().get(0);

        assertThat(beforeGroup.troubleDayCounts())
                .containsExactlyInAnyOrderEntriesOf(Map.of(
                        "깜빡함", 1,
                        "설명을 놓침", 1
                ));
        assertThat(beforeGroup.troubleTypeCounts())
                .containsExactly(Map.entry("INATTENTION", 2));
    }

    private UserMedication medication(
            Long userMedicationId,
            Long medicationId,
            Long dosageId,
            String name,
            String amount,
            LocalDate startedAt
    ) {
        Medication medication = Medication.builder()
                .id(medicationId)
                .name(name)
                .build();
        MedicationDosage dosage = MedicationDosage.builder()
                .id(dosageId)
                .medication(medication)
                .amount(new BigDecimal(amount))
                .build();
        return UserMedication.builder()
                .id(userMedicationId)
                .medicationDosage(dosage)
                .isActive(true)
                .startedAt(startedAt)
                .build();
    }

    private JournalTagLogView troubleLog(
            Long id, String name, String tagType, LocalDate date
    ) {
        JournalTag tag = JournalTag.builder()
                .id(id)
                .category(JournalTagCategory.TROUBLE)
                .name(name)
                .tagType(tagType)
                .scope(JournalTagScope.SYSTEM)
                .ownerKey(JournalTag.SYSTEM_OWNER_KEY)
                .isActive(true)
                .defaultVisible(false)
                .createdAt(LocalDateTime.now())
                .updatedAt(LocalDateTime.now())
                .build();
        JournalTagLog log = JournalTagLog.builder()
                .id(id + 100L)
                .userId(USER_ID)
                .journalTagId(id)
                .journalDate(date)
                .checkedAt(date.atTime(10, 0))
                .build();
        return new JournalTagLogView(log, tag);
    }
}
